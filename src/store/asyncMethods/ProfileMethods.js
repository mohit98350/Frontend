...
export const updateNameAction = (user)=>{
    return async(dispatch,getState)=>{
        ...
        try {
            const {data}= await axios.post('https://express-server1.herokuapp.com/updateName',user,config);
...
export const updatePasswordAction = (userData)=>{
    ...
    dispatch({type:SET_LOADER});
    try {
        const {data}=await axios.post('https://express-server1.herokuapp.com/updatePassword',userData,config)
        dispatch({type:CLOSE_LOADER});
        dispatch({type:SET_MESSAGE,payload:data.msg})
        dispatch({type:REDIRECT_TRUE});
    } catch (error) {
